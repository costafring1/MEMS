...
# Collect Fork bifurcations (label 4)
df_fork = df_labels[df_labels['class_label']==4].copy()


# Check they all have the same length
assert len(df_fold) == len(df_hopf)
assert len(df_hopf) == len(df_branch)
assert len(df_branch) == len(df_fork)
assert len(df_fork) == len(df_null)
...
df_fork['dataset_ID'] = group_nums

# Concatenate dataframes and select relevant columns
df_groups = pd.concat([df_fold,df_hopf,df_branch,df_null,df_fork])[['sequence_ID','dataset_ID']]
